...
// CryptoJS key and iv value for encryption 
...
// Edit profile just takes id email and username to display on page
...
//loads the checkout page and checks if remember is true to load the saved payment information
...
//takes the payment information and stores it in the database
//checks the payment method to get the correct data and if remember is true to store the information
...
//after checking out is submitted it removes, then later adds a robot to the database from the correct station
...
// page after checking out to show the order number and total
app.get('/confirmation', sessionValidation, async (req, res) => {
    ...
    // if statement and order in session was generated by chat gpt
    //author of that part of this code snippet: ChatGPT
    //checks if order hasnt been created yet and creates it
    //order gets cleared when visitiong checkout page
    //order prevents multiple re-orders by refreshing the page
...
// handles the email sending and code generation for the forgot password page
...
//logic for checking the code and redirecting to the reset password page
...
//gets data needed for correct station to be displayed for business owner
...
//adds a robot to the database and sends data to page so an order can be created
...
//creates an order for the business owner, doesnt have order in session to prevent mutliple orders *yet* (could be added, not enough time)
...
//was used in the before launch page, here to be submitted
//for each loop added by ChatGPT
//author of that part of this code snippet: ChatGPT
...
// Fetch orders data based on customer ID. 
//Generated by chat gpt 3.5
//@author https://chat.openai.com/
